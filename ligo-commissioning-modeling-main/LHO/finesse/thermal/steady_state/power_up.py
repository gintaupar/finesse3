...
# %%
Parms = np.linspace(10e3, 800e3, 20)
    ...
    with lho.temporary_parameters():
        for _P in Parms:
            ...
            sols.append(
                lho.run(
                    fac.Series(
                            fac.RunLocks(max_iterations=200, exception_on_fail=False),
                            fac.Noxaxis(),
                            fac.Change({"DARM.DC": 0.005}, relative=True),
                            fac.FrequencyResponse(
                                np.geomspace(1, 5e3, 20),
                                "DARM.AC.i", "AS.DC.o"
                            ),
                        )
                    )
...
for sols in [sols_full, sols_half, sols_zero]:
    N = len(sols)
    sol = SolutionSet([_['noxaxis'] for _ in sols])
...
plt.title("LHO - Build up vs scaled ITM substrate lensing\nIdealised quadratic deformations")
plt.savefig("./figures/power_up/Parm.pdf")
...
plt.title("LHO - Build up vs ITM lensing\nPurely quadratic deformations, No TCS")
plt.savefig("./figures/power_up/PRG.pdf")
...
plt.title("LHO - Build up vs ITM lensing\nPurely quadratic deformations, No TCS")
plt.savefig("./figures/power_up/PRG9.pdf")

# %%
for sols in [sols_full, sols_half, sols_zero]:
    N = len(sols)
    sol = SolutionSet([_['frequency_response'] for _ in sols])
    DC  = SolutionSet([_['noxaxis'] for _ in sols])
    f = sol.f[0, :]
    H = sol.out.squeeze()/np.sqrt(DC['Pas'].solutions)[:,None]
    #plt.loglog(f, abs(H.T))
    plt.semilogy(Pin, abs(H[:, 18]))
# %%
