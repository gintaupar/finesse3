...
def print_params(newpop, i):
    for path, item in newpop.items():
        ...
        if p[0] in ('beam_waist', 'absorption', 'RH_eff', 'P_lens_addl', 'P_surfdef_addl'):
...
def apply_param_variations(factory, param_variations, i):
    ...
    P_addl_ITMlenses = {'ITMX': 0, 'ITMY': 0}
    P_surfdef_addl = {"ITMX": 0, "ETMX": 0, "ITMY": 0, "ETMY": 0}
    ...
    for path, item in param_variations.items():
        ...
        if p[0] == 'beam_waist':
            ...
            if ("w0" in p[1]) and (waist_params[p[1]] < 0):
                # if waist goes negative in sampling, give very small value
                waist_params[p[1]] = 1e-4
            continue
            ...
            # constraining RH_eff between 0 and 1
            if RH_efficiency[p[1]] < 0:
                RH_efficiency[p[1]] = 0
            elif RH_efficiency[p[1]] > 1:
                RH_efficiency[p[1]] = 1
            continue

        elif p[0] == 'P_lens_addl':
            print(f"new additional lens at {p[1]}: {item['vals'][i]}")
            P_addl_ITMlenses[p[1]] = item['vals'][i]
            if P_addl_ITMlenses[p[1]] < 0:
                # allowing negative values of P_addl_ITMlenses as well
                pass
            continue

        elif p[0] == 'P_surfdef_addl':
            print(f"new additional surface deformation at {p[1]}: {item['vals'][i]}")
            P_surfdef_addl[p[1]] = item['vals'][i]
            if P_surfdef_addl[p[1]] < 0:
                # allowing negative values of P_surfdef_addl as well
                pass
            continue
    ...
    return factory, waist_params, losses, TM_absorptions, RH_efficiency, P_addl_ITMlenses, P_surfdef_addl
...
def undo_param_variations(factory, param_variations, i):
    ...
    for path, item in param_variations.items():
        ...
        if p[0] in ('beam_waist', 'loss', 'absorption', 'RH_eff', 'P_lens_addl', 'P_surfdef_addl'):
            continue
...
def runligo(factory,
    ...
    set_ringheaters(llo, factory, "X", P_RH_ITMX, P_RH_ETMX,
                    P_ITMlens_addl=P_addl_ITMlenses["ITMX"],
                    P_ITMsurfdef_addl = P_surfdef_addl["ITMX"],
                    P_ETMsurfdef_addl = P_surfdef_addl["ETMX"],)
    set_ringheaters(llo, factory, "Y", P_RH_ITMY, P_RH_ETMY,
                    P_ITMlens_addl=P_addl_ITMlenses["ITMY"],
                    P_ITMsurfdef_addl = P_surfdef_addl["ITMY"],
                    P_ETMsurfdef_addl = P_surfdef_addl["ETMY"],)

    llo = set_cold_state_beam(llo, update_factory=None, waist_params=waist_params, losses=losses, is_astig=is_astig)
...
def run_population(param_variations,
                   ...
                   use_real_data=False,
                   new_bs=True,
                   thermal_evol_time=5400,
                   cold_evol_time=60,
                   **kwargs):

    # Load in the parameter file to make the
    if new_bs:
        factory = ALIGOBeamSplitterFactory(f"{repopath}/LLO/yaml/llo_O4.yaml")
    else:
    ...
    for i in range(idstart, idend):
        ...
        factory, waist_params, losses, TM_absorptions, RH_efficiency, P_addl_ITMlenses, P_surfdef_addl = apply_param_variations(factory, param_variations, i)
        ...
        if run_thermal:
            t, _, _, _, _, runsols = run_thermal_model(factory=factory,
                                                    ...
                                                    P_addl_ITMlenses=P_addl_ITMlenses,
                                                    P_surfdef_addl=P_surfdef_addl,
                                                    is_astig=is_astig,
...
def set_cold_state_beam(llo, update_factory=None, waist_params={}, losses={}, is_astig=False):
    ...
    # giving sample w0 and z which will be overriden
    wkey, zkey, astigParams = get_astig_params(w0=1.015e-3, z=6.0, is_astig=is_astig)
...
def set_ringheaters(llo, factory, arm, P_RH_ITM, P_RH_ETM, P_ITMlens_addl=0, P_ITMsurfdef_addl=0, P_ETMsurfdef_addl=0):
    lens = llo.get(f"ITM{arm}lens")
    lens.f = 1 / (1 / lens.f + P_RH_ITM * factory.params.IRH_sub + P_ITMlens_addl)
    itm = llo.get(f"ITM{arm}")
    etm = llo.get(f"ETM{arm}")
    itm.Rc = 2 / (2 / itm.Rc + P_RH_ITM * factory.params.IRH_srf + P_ITMsurfdef_addl)
    etm.Rc = 2 / (2 / etm.Rc + P_RH_ETM * factory.params.ERH_srf + P_ETMsurfdef_addl)
    print(f"New values for ITM{arm} " + \
          f"lens f: {lens.f.eval()}, ITM{arm} Rc: {itm.Rcx.eval()}, " + \
          f"ETM{arm} Rc: {etm.Rcx.eval()}")

def pre_make_update_yaml(factory, yaml):
    factory.update_parameters(yaml)
    return

def set_addl_ITMlenses_RHs(llo, factory, RH_efficiency, P_addl_ITMlenses, P_surfdef_addl):

    print(f"Setting RH_efficiency: {RH_efficiency}\nSetting P_addl_ITMlenses: {P_addl_ITMlenses}\nSetting P_surfdef_addl: {P_surfdef_addl}")
    # RH powers
    P_RH_ITMX = factory.params.P_RH_ITMX * RH_efficiency['ITMX']
    P_RH_ITMY = factory.params.P_RH_ITMY * RH_efficiency['ITMY']
    P_RH_ETMX = factory.params.P_RH_ETMX * RH_efficiency['ETMX']
...
def post_make_update_yaml(llo,
    ...
    # set waist parameters from factory updated with yaml
    llo = set_cold_state_beam(llo, update_factory=factory, waist_params=waist_params, losses=losses, is_astig=is_astig)

    # RH efficiencies
    RH_efficiency={}
    for TMkey in factory.params.RH_eff:
        RH_efficiency[TMkey] = factory.params.RH_eff[TMkey]
        # restricting in correct range
        if RH_efficiency[TMkey] > 1:
            RH_efficiency[TMkey] = 1
        elif RH_efficiency[TMkey] < 0:
            RH_efficiency[TMkey] = 0

    llo = set_addl_ITMlenses_RHs(llo, factory, RH_efficiency, P_addl_ITMlenses, P_surfdef_addl)
...
def run_thermal_model(factory=None,
    ...
    initial_data_time: int
        Start time for the real power up data that is used if use_real_data is True. Default is 1389224700.
    override_RHeff: bool
        if True, ignore yaml input for RH efficiency and override with kwarg input.
    P_addl_ITMlenses: dict
        Additional const lens at ITMs in optical power (uD).
        Default is {'ITMX': 0, 'ITMY': 0}
    P_surfdef_addl: dict
        Additional const surface deformation at TMs in optical power (uD).
        Default is {"ITMX": 0, "ETMX": 0, "ITMY": 0, "ETMY": 0}
    addl_parse: string
    ...
    if update_yaml:
        pre_make_update_yaml(factory, update_yaml)
    ...
    if update_yaml:
        llo = post_make_update_yaml(llo,
                                    factory=factory,
                                    waist_params={},
                                    losses={},
                                    is_astig=is_astig,
                                    P_addl_ITMlenses=factory.params.P_lens_addl,
                                    P_surfdef_addl=factory.params.P_surfdef_addl,
                                    )
        tm_absorption = factory.params.absorption
    else:
        llo = set_cold_state_beam(llo, update_factory=None, waist_params=waist_params, losses=losses, is_astig=is_astig)

    # override RH_efficiency from factory params
    # or if it is a regular run without yaml, set RH and addl lenses
    if override_RHeff or not update_yaml:
        llo = set_addl_ITMlenses_RHs(llo, factory, RH_efficiency, P_addl_ITMlenses, P_surfdef_addl)
    ...
    def update_ts(base1, L0Pmax=64, use_real_data=False):
        ...
        if use_real_data:
            ...
            print('Current time is {}s, power input is {}W.'.format(ts_itmx.t,current_P))
...
def get_actual_params(all_sols,
    ...
    for ii in range(len(all_sols)):
        ...
        for path, item in newpop.items():
            ...
            elif p[0] in ('loss', 'absorption', 'RH_eff', 'P_lens_addl', 'P_surfdef_addl'):
                data[ii][path] = item['vals']
                continue
...
def print_yamlparams_factory(factory, yamlparams):
    for path in yamlparams.keys():
        p = path.split('/')
        if p[0] in ('beam_waist', 'absorption', 'RH_eff', 'P_lens_addl', 'P_surfdef_addl'):
            continue
        if len(p) == 2:
            print(f"{p[0]} {p[1]}: {factory.params[p[0]][p[1]]}")
        elif len(p) == 3:
            print(f"{p[0]} {p[1]} {p[2]}: {factory.params[p[0]][p[1]][p[2]]}")
