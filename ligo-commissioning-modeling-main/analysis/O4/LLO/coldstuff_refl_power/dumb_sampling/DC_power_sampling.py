...
model.gIM2_p1_i.w0y = model.gIM2_p1_i.w0x.ref
model.gIM2_p1_i.zy = model.gIM2_p1_i.zx.ref
...
for i in range(1001):
    with model.temporary_parameters():
        model.IM2.Rc += np.random.uniform(-100e-3, +100e-3)
        model.IM3.Rc += np.random.uniform(-100e-3, +100e-3)
        
        model.PR3.Rc += np.random.uniform(-15e-3, +15e-3)
        model.PR2.Rc += np.random.uniform(-15e-3, +15e-3)
        dL = np.random.uniform(-5e-3, +5e-3)
        model.lp2.L += dL
        model.lp3.L -= dL
        ...
        model.ITMX.Rc = 2 / (2 / abs(base.ITMX.Rc) + RH_I_uD_W * P_IX_RH)
        model.ETMX.Rc = 2 / (2 / abs(base.ETMX.Rc) + RH_E_uD_W * P_EX_RH)
        model.ITMY.Rc = 2 / (2 / abs(base.ITMY.Rc) + RH_I_uD_W * P_IY_RH)
        model.ETMY.Rc = 2 / (2 / abs(base.ETMY.Rc) + RH_E_uD_W * P_EY_RH)

        model.ITMXlens.f = 1/(1/base.ITMXlens.f + RH_f_uD_W * P_IX_RH)
        model.ITMYlens.f = 1/(1/base.ITMYlens.f + RH_f_uD_W * P_IY_RH)

        try:
            sol = model.run(fa.Series(*actions))
            
            DC = sol['DC']
            samples["IM2_Rc"].append(model.IM2.Rcx.value)
            samples["IM3_Rc"].append(model.IM3.Rcx.value)
            samples["PR3_Rc"].append(model.PR3.Rcx.value)
            samples["PR2_Rc"].append(model.PR2.Rcx.value)
            samples["dL[mm]"].append(dL/1e-3)
            samples["P_IX_RH"].append(P_IX_RH)
            samples["PreflPRM"].append( DC['PreflPRM'])
            samples["E_car_refl"].append( DC['E_car_refl'])
            samples["E_car_prc"].append( DC['E_car_prc'])
            samples["E_u9_refl"].append( DC['E_u9_refl'])
            samples["E_u9_prc"].append( DC['E_u9_prc'])
            samples["E_l9_refl"].append( DC['E_l9_refl'])
            samples["E_l9_prc"].append( DC['E_l9_prc'])
            samples["PRG"].append( DC['PRG'])
            samples["AGX"].append( DC['AGX'])
            samples["AGY"].append( DC['AGY'])
            samples["Px"].append( DC['Px'])
            samples["Py"].append( DC['Py'])
            samples["Pprc"].append( DC['Pprc'])
            samples["Gouy_PRX"].append(model.cavPRX.gouy/2)
            samples["Gouy_PRY"].append(model.cavPRY.gouy/2)
            samples["Gouy_SRX"].append(model.cavSRX.gouy/2)
            samples["Gouy_SRY"].append(model.cavSRY.gouy/2)
            print(f"Rank {rank} finished iteration {i}", flush=True)
        except:
            print(f"Rank {rank} skip iteration {i}", flush=True)
        finally:
            if i % 100 == 0 and i > 0:
                np.savez_compressed(f"samples_{rank}_{i}.py", **{k:np.array(v) for k,v in samples.items()})
                samples.clear()
